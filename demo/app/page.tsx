...
import { FileText, Camera, Globe, MessageCircle, LayoutDashboard, CalendarDays, ChevronRight } from "lucide-react";
  ...
  return (
    <main className="min-h-screen bg-cream text-ink flex items-center justify-center p-5 sm:p-6">
      <div className="max-w-xl w-full min-w-0">
        ...
        {/* Entregáveis pro cliente: site + Instagram */}
        <div className="grid sm:grid-cols-2 gap-3 mt-4">
          <Link
            href="/site"
            className="bg-card rounded-3xl border border-brand/30 p-5 flex items-center gap-3 hover:border-brand hover:shadow-md transition"
          >
            <Globe size={26} className="text-brand" />
            <div className="flex-1">
              <div className="font-bold">Site institucional</div>
              <div className="text-muted text-sm">A página pública da clínica.</div>
            </div>
          ...
          >
            ...
            <div className="flex-1">
              <div className="font-bold">Instagram pronto</div>
              <div className="text-muted text-sm">Feed @clinicarotelli com 12 posts.</div>
            </div>
            <ChevronRight size={20} className="text-brand" />
          </Link>
        </div>
